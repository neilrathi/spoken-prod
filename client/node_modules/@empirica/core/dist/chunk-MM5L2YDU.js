...
import { State as State2 } from "@empirica/tajriba";
...
import { z } from "zod";
var isString = z.string().parse;
var isOptionalNumber = z.number().optional().parse;
...
var lobbyConfigSchema = z.object({
...
var BatchOwned = class extends Scope {
  get batch() {
...
var Round = class extends GameOwned {
  ...
  addStage(attributes) {
    ...
    const res = z.number().int().gte(minStageDuration).lte(maxStageDuration).safeParse(durAttr?.value);
    if (!res.success) {
      throw new Error(`stage duration invalid: ${res.error}`);
    }
    const duration = res.data;
...
var treatmentSchema = z2.record(z2.string().min(1), z2.any());
var batchConfigSchema = z2.discriminatedUnion("kind", [
  z2.object({
    kind: z2.literal("custom"),
    config: z2.any()
  }),
  z2.object({
    kind: z2.literal("simple"),
    config: z2.object({
      count: z2.number().int().positive(),
      treatments: z2.object({
        factors: treatmentSchema
      }).array()
    })
  }),
  z2.object({
    kind: z2.literal("complete"),
    config: z2.object({
      treatments: z2.object({
        count: z2.number().int().positive(),
        treatment: z2.object({
...
// src/admin/classic/classic.ts
var isGame = z3.instanceof(Game).parse;
var isRound = z3.instanceof(Round).parse;
var isStage = z3.instanceof(Stage).parse;
var isString2 = z3.string().parse;
function Classic({
  disableAssignment,
  disableIntroCheck,
  disableGameCreation,
  ...
  return function(_) {
    ...
    _.on(
      ...
      (_2, { step, transition: { from, to } }) => {
        ...
        if (from === State2.Running && to === State2.Ended) {
...
var Scope2 = class {
  constructor(conn, edge) {
    ...
    for (const edge2 of this.edge.attributes.edges) {
      this._attributes.set(edge2.node.key, new Attribute(edge2.node));
    }
  }
  get id() {
    return this.edge.id;
...
var Player3 = class extends Scope2 {
};
var Batch3 = class extends Scope2 {
};
var Game2 = class extends Scope2 {
};
var PlayerGame2 = class extends Scope2 {
};
var Round2 = class extends Scope2 {
};
var PlayerRound2 = class extends Scope2 {
};
var Stage2 = class extends Scope2 {
};
var PlayerStage3 = class extends Scope2 {
};
var Conn = class {
  constructor(tajriba) {
...
async function exportCSV(conn, output) {
  ...
  const stream = fs2.createWriteStream(output);
...
import { State as State3 } from "@empirica/tajriba";
  ...
  return function(_2) {
    ...
    _2.on(
      ...
      (ctx, { step, transition: { from, to } }) => {
        ...
        if (from !== State3.Running || to !== State3.Ended) {
          return;
        }
...
tmp/lib/tmp.js:
   ...
   *)
*/
//# sourceMappingURL=chunk-MM5L2YDU.js.map